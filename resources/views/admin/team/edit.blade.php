    ...
    <div class="row">
        ...
        <div class="col-md-12">
            <div class="card">
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <form action="{{ route('admin.team.update', ['team' => $team->id]) }}" method="put"  id="popup-form-team" >
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label>Title</label>
                            <input type="text" name="title" id="title" value="{{$team->title}}" class="form-control" required autocomplete="title" autofocus maxlength="60">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control select2" id="team_leader" name="team_leader" required autocomplete="team_leader">
                                @foreach ($users as $user)
                                    <option value="{{ $user->id }}" @if($team->team_leader == $user->id) selected="selected" @endif>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group">
                            <label>Team Members &nbsp;</label><input type="checkbox" id="checkbox_user" > &nbsp;Select All
